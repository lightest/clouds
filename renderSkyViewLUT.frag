...
float fromSubUvsToUnit(float u, float resolution) {
  return (u - .5 / resolution) * (resolution / (resolution - 1.));
}
...
void uvToSkyViewLutParams(out float viewZenithCosAngle, out float lightViewCosAngle, in float viewHeight, in vec2 uv) {
  ...
  uv = vec2(fromSubUvsToUnit(uv.x, uSkyViewLUTSize.x), fromSubUvsToUnit(uv.y, uSkyViewLUTSize.y));
...
vec3 marchSkyLayer (vec3 marchOrigin, vec3 rayDir, vec3 dirToLight, float marchDist, float steps) {
  vec3 samplePos;
  ...
  return scatteredLightIntegrated;
}

void marchAtmosphere (vec3 cameraPos, vec3 rayDir, vec3 sunDir) {
  float RPlanetDist;
  float R1Dist;
  float R1DistFar;
  ...
  vec3 skyLayerScatteredLight = marchSkyLayer(skyMarchOrigin, rayDir, sunDir, skyMarchDist, uSkyMarchSamples);
  color = vec4(skyLayerScatteredLight, 1.);
}
...
void main () {
  ...
  uvToSkyViewLutParams(viewZenithCosAngle, lightViewCosAngle, viewHeight, vTexCoord);
  vec3 upVector = cameraPos / viewHeight;
  float sunZenithCosAngle = dot(upVector, normalize(uLightPos));
  vec3 sunDir = normalize(vec3(sqrt(1. - sunZenithCosAngle * sunZenithCosAngle), 0., sunZenithCosAngle));

  cameraPos = vec3(0., 0., viewHeight);

  float viewZenithSinAngle = sqrt(1. - viewZenithCosAngle * viewZenithCosAngle);
  rayDir = vec3(
    viewZenithSinAngle * lightViewCosAngle,
    viewZenithSinAngle * sqrt(1. - lightViewCosAngle * lightViewCosAngle),
    viewZenithCosAngle);
  marchAtmosphere(cameraPos, rayDir, sunDir);
}
