...
var FluidSim = (function () {
  ...
  let SIM_DIMENSIONS = new Float32Array([78, 78, 78]);
  let VISCOSITY = .001;
  const DX = 1.;
  ...
  const PRESSURE_BETA_3D = 1/6;
  let DIFFUSION_ALPHA;
  let DIFFUSION_BETA;
  let DIFFUSION_BETA_3D;
  let MOUSEBUTTONS = {
    LMB: 1,
    RMB: 3,
    MMB: 2
  ...
  class FluidSim {
    ...
    init (params) {
      ...
      this._programs.slabOP_applyForce = new ShaderProgram({
        vertPath: './renderToTexture.vert',
        fragPath: './fluid/3d/applyForce.frag',
        gl: this._gl,
      ...
      this._programs.slabOP_injectInk = new ShaderProgram({
        vertPath: './renderToTexture.vert',
        fragPath: './fluid/3d/injectInk.frag',
        gl: this._gl,
      ...
      this._programs.slabOP_divergence = new ShaderProgram({
        vertPath: './renderToTexture.vert',
        fragPath: './fluid/3d/divergence.frag',
        gl: this._gl,
      ...
      this._programs.slabOP_subtractGradient = new ShaderProgram({
        vertPath: './renderToTexture.vert',
        fragPath: './fluid/3d/subtractGradient.frag',
        gl: this._gl,
      ...
      this._programs.vorticityImpact = new ShaderProgram({
        vertPath: './renderToTexture.vert',
        fragPath: './fluid/3d/applyVorticity.frag',
        gl: this._gl,
    ...
    _setupAdvectProgram (gl, shaderProgram) {
      ...
      this._gl.bindTexture(this._gl.TEXTURE_3D, this._simTextures.currentVelocityTexture);
    }
    ...
    _setupApplyForceProgram (gl, shaderProgram) {
      ...
      gl.uniform1f(shaderProgram.unifs.uForceSplatSize, this.forceSplatSize);
    }

    _setupDivergenceProgram (gl, shaderProgram) {
      gl.enableVertexAttribArray(shaderProgram.attrs.aPos);
      gl.enableVertexAttribArray(shaderProgram.attrs.aTexCoord);
      gl.uniform1i(shaderProgram.unifs.uVelocityVol, 0);
    }
    ...
    _setupSimTextures () {
      ...
      this._simTexWidth = SIM_DIMENSIONS[0];
      this._simTexHeight = SIM_DIMENSIONS[1];
      if (aspect > 1) {
        this._simTexHeight = Math.floor(this._simTexHeight / aspect);
      } else {
        this._simTexWidth = Math.floor(this._simTexWidth * aspect);
      }
    ...
    _setupSimTextures3D () {
      ...
      for (i in this._simTextures) {
        this._gl.deleteTexture(this._simTextures[i]);
        this._simTextures[i] = this._gl.createTexture();
        this._gl.bindTexture(this._gl.TEXTURE_3D, this._simTextures[i]);
      ...
      this._gl.deleteTexture(this._inkTex);
      this._inkTex = this._gl.createTexture();
      this._gl.bindTexture(this._gl.TEXTURE_3D, this._inkTex);
      this._gl.texParameteri(this._gl.TEXTURE_3D, this._gl.TEXTURE_MAG_FILTER, this._gl.LINEAR);
      this._gl.texParameteri(this._gl.TEXTURE_3D, this._gl.TEXTURE_MIN_FILTER, this._gl.LINEAR);
      this._gl.texParameteri(this._gl.TEXTURE_3D, this._gl.TEXTURE_WRAP_S, this._gl.REPEAT);
      this._gl.texParameteri(this._gl.TEXTURE_3D, this._gl.TEXTURE_WRAP_T, this._gl.REPEAT);
      ...
      this._gl.deleteTexture(this._inkTexSwap);
      this._inkTexSwap = this._gl.createTexture();
      this._gl.bindTexture(this._gl.TEXTURE_3D, this._inkTexSwap);
      this._gl.texParameteri(this._gl.TEXTURE_3D, this._gl.TEXTURE_MAG_FILTER, this._gl.LINEAR);
      this._gl.texParameteri(this._gl.TEXTURE_3D, this._gl.TEXTURE_MIN_FILTER, this._gl.LINEAR);
      this._gl.texParameteri(this._gl.TEXTURE_3D, this._gl.TEXTURE_WRAP_S, this._gl.REPEAT);
      this._gl.texParameteri(this._gl.TEXTURE_3D, this._gl.TEXTURE_WRAP_T, this._gl.REPEAT);
    ...
    _calcCurl3D () {
      ...
      this._gl.activeTexture(this._gl.TEXTURE0);
      this._gl.bindTexture(this._gl.TEXTURE_3D, this._simTextures.currentVelocityTexture);
      this._gl.uniform1f(this._currentProgram.unifs.uLayersTotal, SIM_DIMENSIONS[2] * 2);
      let i;
      for (i = 0; i < SIM_DIMENSIONS[2]; i++) {
        ...
        this._gl.uniform1f(this._currentProgram.unifs.uLayer, 1 + 2 * i);
        this._gl.drawElements(this._gl.TRIANGLES, this._indices.length, this._gl.UNSIGNED_INT, 0);
      }
    }
    ...
    _applyVorticityImpact3D () {
      this._setCurrentProgram(this._programs.vorticityImpact);
      this._setupRenderingToBufferTexture();
      this._gl.activeTexture(this._gl.TEXTURE0);
      this._gl.bindTexture(this._gl.TEXTURE_3D, this._simTextures.currentVelocityTexture);
      this._gl.activeTexture(this._gl.TEXTURE1);
      this._gl.bindTexture(this._gl.TEXTURE_3D, this._simTextures.curlTexture);
      this._gl.uniform1f(this._currentProgram.unifs.uLayersTotal, SIM_DIMENSIONS[2] * 2);
      let i;
      for (i = 0; i < SIM_DIMENSIONS[2]; i++) {
        this._gl.framebufferTextureLayer(
    ...
    _applyForce3D () {
      ...
      this._gl.uniform2fv(this._currentProgram.unifs.uForceDirection, this._forceDirection);
      this._gl.uniform1f(this._currentProgram.unifs.uLayersTotal, SIM_DIMENSIONS[2] * 2);
      this._gl.uniform1i(this._currentProgram.unifs.uUseZv, this._useZv);
      let i;
      for (i = 0; i < SIM_DIMENSIONS[2]; i++) {
        this._gl.framebufferTextureLayer(
          this._gl.FRAMEBUFFER,
    ...
    _doAdvect (outTexture, textureToAdvect, width = SIM_DIMENSIONS[0], height = SIM_DIMENSIONS[1], fbo = this._framebuffer, fadeFactor = 1.) {
    ...
    _doAdvect3D (outVolume, volumeToAdvect, width = SIM_DIMENSIONS[0], height = SIM_DIMENSIONS[1], fbo = this._framebuffer, fadeFactor = 1.) {
      this._setCurrentProgram(this._programs.slabOP_advect);
      this._setupRenderingToBufferTexture(width, height, fbo);
      this._gl.activeTexture(this._gl.TEXTURE1);
      this._gl.bindTexture(this._gl.TEXTURE_3D, volumeToAdvect);
      this._gl.uniform1f(this._currentProgram.unifs.uFadeFactor, fadeFactor);
      this._gl.uniform1f(this._currentProgram.unifs.uLayersTotal, SIM_DIMENSIONS[2] * 2);
      let i;
      for (i = 0; i < SIM_DIMENSIONS[2]; i++) {
    ...
    _doDiffusion3D (n) {
      ...
      this._setCurrentProgram(this._programs.slabOP_jacobi);
      this._setupRenderingToBufferTexture();
      this._gl.uniform1f(this._currentProgram.unifs.uBeta, DIFFUSION_BETA_3D);
      this._gl.uniform1f(this._currentProgram.unifs.uLayersTotal, SIM_DIMENSIONS[2] * 2);
      for (i = 0; i < n; i++) {
        this._gl.activeTexture(this._gl.TEXTURE0);
        this._gl.bindTexture(this._gl.TEXTURE_3D, this._simTextures.currentVelocityTexture);
        ...
        for (j = 0; j < SIM_DIMENSIONS[2]; j++) {
          ...
          this._gl.uniform1f(this._currentProgram.unifs.uLayer, 1 + 2 * j);
          this._gl.drawElements(this._gl.TRIANGLES, this._indices.length, this._gl.UNSIGNED_INT, 0);
        }
        this._swapVelocities();
      }
    ...
    _doPressure3D (n) {
      ...
      this._setupRenderingToBufferTexture();
      this._gl.activeTexture(this._gl.TEXTURE1);
      this._gl.bindTexture(this._gl.TEXTURE_3D, this._simTextures.divergenceTexture);
      this._gl.uniform1f(this._currentProgram.unifs.uAlpha, PRESSURE_ALPHA);
      this._gl.uniform1f(this._currentProgram.unifs.uBeta, PRESSURE_BETA_3D);
      this._gl.uniform1f(this._currentProgram.unifs.uLayersTotal, SIM_DIMENSIONS[2] * 2);
      for (i = 0; i < n; i++) {
        this._gl.activeTexture(this._gl.TEXTURE0);
        this._gl.bindTexture(this._gl.TEXTURE_3D, this._simTextures.currentPressureTexture);
        for (j = 0; j < SIM_DIMENSIONS[2]; j++) {
          this._gl.framebufferTextureLayer(
            this._gl.FRAMEBUFFER,
            this._gl.COLOR_ATTACHMENT0,
    ...
    _doPressureGradientSubtraction3D () {
      let i;
      this._setCurrentProgram(this._programs.slabOP_subtractGradient);
      this._setupRenderingToBufferTexture();
      this._gl.activeTexture(this._gl.TEXTURE0);
    ...
    _runSim () {
      ...
      this._doAdvect3D(this._simTextures.swapVelocityTexture, this._simTextures.currentVelocityTexture);
      this._swapVelocities();
      this._doAdvect3D(this._inkTexSwap, this._inkTex, this._inkTexWidth, this._inkTexHeight, this._inkFBO, this.inkFadeFactor);
      let swap = this._inkTex;
      this._inkTex = this._inkTexSwap;
      this._inkTexSwap = swap;
      if (this._applyingForce) {
      ...
      if (this._useVorticity) {
        this._calcCurl3D();
        this._applyVorticityImpact3D();
      }
      if (this._injectingInk) {
        this._injectInk3D();
      }
      this._doDiffusion3D(this.diffusionIterations);
      this._doDivergence3D(this._simTextures.divergenceTexture);
      this._doPressure3D(this.pressureIterations);
      this._doPressureGradientSubtraction3D();
    }
    ...
    update (dt) {
      ...
      DIFFUSION_BETA_3D = 1 / (6 + DIFFUSION_ALPHA);
    }
